...
Instructions for setting up backups [are available here](https://github.com/bitembassy/home-node/blob/master/lightning-backup.md), including optional encrtypted cloud backups to Keybase.
...
You will need to find the IP range of your local network. It should look something like `192.168.0.0/24` - in most cases, find your own local IP, change the last cell to `0` and add `/24`. For example, `10.10.5.17` means IP range 10.10.5.**0/24**. 
