...
Find your Master Public Key in elecrum wallet (Wallet > Information) and add it to `eps.cfg` under `[master-public-keys]` as a new line with `{name}={xpubkey}`. `{name}` can be anything. (Uncomment and replace sample xpubkey).
